class LzwDecoder:
    ...
    def decode(self):
        ...
        while True:
            current_code, bit_buffer, bits_in_buffer = self._read_next_code_from_block(
            ...
            if current_code == self.clear_code:
                dictionary = {i: [i] for i in range(self.clear_code)}
                self._reset_parameters_after_clear_code()
                old_code = None
                continue
            elif current_code == self.end_code:
                break

            entry, dictionary, old_code = self._process_code(current_code, dictionary, old_code, codes)

        return codes

    def _reset_parameters_after_clear_code(self):
        self.code_size = self.min_code_size + 1
        self.next_code = self.clear_code + 2

    @staticmethod
    def _read_next_code_from_block(bit_buffer, bits_in_buffer, code_size, data_iter):
    ...
    def _process_code(self, current_code, dictionary, old_code, codes):
